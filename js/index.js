...
function mytimeshforid(wheretoshow) {
  ...
  var pretty_time = `${String(current_date.getHours()).padStart(2, '0')}:${String(
...
function gameControls(operation) {
  switch (operation) {
    ...
    case 'buttonDivide':
      ...
      break;
    case 'readfile':
      readingFileWithGenes();
      break;
  }
  //    document.getElementById("rezultPlace").innerHTML = rezult;
}
//------------------promise start-------------------------------
let isthereSaveddata = new Promise((resolve, reject) => {
  alert('Preparing data');
  if (localStorage.hasOwnProperty('genestotransfer')) {
    transferdata = JSON.parse(localStorage.getItem('genestotransfer'));
    resolve(transferdata);
  } else {
    let errtext = 'No data in localStorage! Will be loaded default values.';
    reject(errtext);
  }
});
isthereSaveddata.then(
  (transferdata) => {
    alert('Previously stored data found in local storage, continue processing');
    return new Promise((resolve, reject) => {
      let tinput1 = input1.value;
      let tinput2 = input2.value;
      if (
        transferdata.hasOwnProperty('input1') &&
        transferdata.hasOwnProperty('input2')
      ) {
        ...
        let errtext1 =
          'In localStorage no data for Input1 or Input2! Will be loaded default values, by the way, you may load your data from save file .genes..';
        input1.value = tinput1;
        input2.value = tinput2;
        reject(errtext1);
      }
    }).then(
      () => {
        alert(
          'Snakes on start and Game speed values are taken from storage, by the way, you may load your data from earlier saved file <>.genes.'
        );
      },
      (errtext) => {
        alert(errtext);
...
SnakeObject = function (
  ...
  this.generateBody = function () {
    ...
    for (let i = 0; i < this.startSnakeLength; i++) {
      this.snakeBody.push(
        new PointOfField(this.headX - i, this.headY, this.headColor)
      );
    }
  };
    ...
    if (Snaketocalkdir.snakeDirection === 0) {
      ...
      for (let i = 1; i < that.eyeDepth; i++) {
        for (let j = -i; j <= i; j++) {
          ...
          Snaketocalkdir.leftEyevalue +=
            eyevalueforcalculation *
            that.checkeyevalue(xn1, yn1, numofsntocalcdir);
          ii++;
        }
      }
      ...
      for (let i = 1; i < that.eyeDepth; i++) {
        for (let j = -i; j <= i; j++) {
          ...
          xn1 = (x1 + that.step * j + 1 + that.gameWidth) % that.gameWidth;
          yn1 = (y1 - that.step * i + that.gameHeight) % that.gameHeight;
          Snaketocalkdir.rightEyevalue +=
            eyevalueforcalculation *
            that.checkeyevalue(xn1, yn1, numofsntocalcdir);
          ii++;
        }
      }
      Snaketocalkdir.snakeDirection =
        (Snaketocalkdir.snakeDirection +
...
gameManager = function () {
  ...
  this.eyevalues = [
    [
      0.707, 1.0, 0.707, 0.353, 0.447, 0.5, 0.447, 0.353, 0.235, 0.277, 0.316,
      0.333, 0.316, 0.277, 0.235, 0.176, 0.2, 0.223, 0.242, 0.25, 0.242, 0.223,
      0.2, 0.176, 0.141, 0.156, 0.171, 0.185, 0.196, 0.2, 0.196, 0.185, 0.171,
      0.156, 0.141, 0.117, 0.128, 0.138, 0.149, 0.158, 0.164, 0.166, 0.164,
      0.158, 0.149, 0.138, 0.128, 0.117, 0.101, 0.108, 0.116, 0.124, 0.131,
      0.137, 0.141, 0.142, 0.141, 0.137, 0.131, 0.124, 0.116, 0.108, 0.101,
      0.088, 0.094, 0.1, 0.105, 0.111, 0.117, 0.121, 0.124, 0.125, 0.124, 0.121,
      0.117, 0.111, 0.105, 0.1, 0.094, 0.088, 0.078, 0.083, 0.087, 0.092, 0.097,
      0.101, 0.105, 0.108, 0.11, 0.111, 0.11, 0.108, 0.105, 0.101, 0.097, 0.092,
      0.087, 0.083, 0.078, 0.07, 0.074, 0.078, 0.081, 0.085, 0.089, 0.092,
      0.095, 0.098, 0.099, 0.1, 0.099, 0.098, 0.095, 0.092, 0.089, 0.085, 0.081,
      0.078, 0.074, 0.07, 0.064, 0.067, 0.07, 0.073, 0.076, 0.079, 0.082, 0.085,
      0.087, 0.089, 0.09, 0.09, 0.09, 0.089, 0.087, 0.085, 0.082, 0.079, 0.076,
      0.073, 0.07, 0.067, 0.064, 0.058, 0.061, 0.064, 0.066, 0.069, 0.071,
      0.074, 0.076, 0.079, 0.08, 0.082, 0.083, 0.083, 0.083, 0.082, 0.08, 0.079,
      0.076, 0.074, 0.071, 0.069, 0.066, 0.064, 0.061, 0.058, 0.054, 0.056,
      0.058, 0.06, 0.063, 0.065, 0.067, 0.069, 0.071, 0.073, 0.074, 0.076,
      0.076, 0.076, 0.076, 0.076, 0.074, 0.073, 0.071, 0.069, 0.067, 0.065,
      0.063, 0.06, 0.058, 0.056, 0.054, 0.05, 0.052, 0.054, 0.056, 0.058, 0.06,
      0.062, 0.063, 0.065, 0.067, 0.068, 0.069, 0.07, 0.071, 0.071, 0.071, 0.07,
      0.069, 0.068, 0.067, 0.065, 0.063, 0.062, 0.06, 0.058, 0.056, 0.054,
      0.052, 0.05, 0.047, 0.048, 0.05, 0.052, 0.053, 0.055, 0.057, 0.058, 0.06,
      0.061, 0.063, 0.064, 0.065, 0.066, 0.066, 0.066, 0.066, 0.066, 0.065,
      0.064, 0.063, 0.061, 0.06, 0.058, 0.057, 0.055, 0.053, 0.052, 0.05, 0.048,
      0.047,
    ],
    [
      0.707, 1.0, 0.707, 0.353, 0.447, 0.5, 0.447, 0.353, 0.235, 0.277, 0.316,
  ...
  this.gameStart = function () {
    if (isloadedJSONoject) {
      ...
      this.genesfromtramsfer = JSON.parse(
        localStorage.getItem('genestotransfer')
      );
      this.eyevalues = [];
    ...
    this.startQuantityOfSnakes = Number(
      document.getElementById('input1').value
    );
    this.gamepause =
      Math.floor(10000 / Number(document.getElementById('input2').value)) < 20
        ? 20
        : Math.floor(10000 / Number(document.getElementById('input2').value));
    this.food = new food();
  ...
  this.renderpicture = function () {
    ...
    for (let ii = 0; ii < this.snakesonfield.length; ii++) {
      for (let j = 0; j < this.snakesonfield[ii].snakeBody.length; j++) {
        let x1 =
          this.snakesonfield[ii].snakeBody[j].getX() *
          this.snakesonfield[ii].cellBodysize;
        let y1 =
          this.snakesonfield[ii].snakeBody[j].getY() *
          this.snakesonfield[ii].cellBodysize;
        let radiusofSnakebodysegment = Math.floor(
          (this.snakesonfield[ii].cellBodysize - 2) / 2
        );
        let my_gradient = null;
        my_gradient = ctx.createRadialGradient(
          x1 +
            Math.floor(
              (Math.random() * 2 - 1) * (radiusofSnakebodysegment / 2)
            ),
          y1 +
            Math.floor(
              (Math.random() * 2 - 1) * (radiusofSnakebodysegment / 2)
    ...
    for (let i = 0; i < this.food.foodvolume.length; i++) {
      ...
      ctx.drawImage(
        this.foodimage,
        x1 - this.food.radiusoffoodportion / 2,
        y1 - this.food.radiusoffoodportion / 2,
        Math.floor(this.food.radiusoffoodportion / 1.4),
        this.food.radiusoffoodportion
      );
    }
    //poison
    for (let i = 0; i < this.poison.poisonvolume.length; i++) {
      let x1 =
        this.poison.poisonvolume[i].getX() * this.poison.radiusofpoisonportion;
      let y1 =
        this.poison.poisonvolume[i].getY() * this.poison.radiusofpoisonportion;
      ctx.drawImage(
        this.poisonimage,
        x1 - this.poison.radiusofpoisonportion / 2,
        y1 - this.poison.radiusofpoisonportion / 2,
        this.food.radiusoffoodportion,
        this.food.radiusoffoodportion
      );
    }
    //game subtotals and other results on web page
    document.getElementById('rezultPlace').innerHTML =
      '' + this.eaten + (this.fight === 1 ? '<br>Peace' : '<br>Fight');
  };
  ...
  this.isgameover = function () {
    ...
    if (
      this.snakesonfield.length > this.maxsSnakeNumber &&
      countcolors.length < 2
    ) {
      document.getElementById('rezultPlace').innerHTML =
        'Game Over<br>' +
        this.eaten +
        (this.fight === 1 ? '<br>Peace' : '<br>Fight');
      this.mutationstotransfer = [];
      for (let i = 0; i < this.savegenes; i++) {
        this.genestotransfer.push(
          this.snakesonfield[this.snakesonfield.length - 1 - i].snakeEyeValues
        );
        this.mutationstotransfer.push(
          this.snakesonfield[this.snakesonfield.length - 1 - i].mutations
        );
      }
      ...
      let filenametoexport =
        'genes ' +
        `${String(current_date.getDate()).padStart(2, '0')}.${String(
          current_date.getMonth() + 1
        ).padStart(2, '0')}.${String(current_date.getFullYear())}-${String(
          current_date.getHours()
        ).padStart(2, '0')}.${String(current_date.getMinutes()).padStart(
          2,
          '0'
        )}`;
  ...
  this.checkandaddsnakes = function () {
    while (this.snakesonfield.length < this.startQuantityOfSnakes) {
      ...
      this.snakesonfield.push(
        new SnakeObject(
          this.startSnakeLength,
          x,
          y,
          this.startDirection,
          fillColor,
          this.cellsize,
          this.eyevalues[this.snakesonfield.length % this.eyevalues.length],
          this.mutationstotransfer.length < 1
            ? this.mutationsonstart
            : this.mutationstotransfer[
                this.snakesonfield.length % this.mutationstotransfer.length
              ]
        )
      );
  ...
  this.isSnakeItself = function (x, y, numofsnaketocompare) {
    for (
      let j = 0;
      j < this.snakesonfield[numofsnaketocompare].snakeBody.length;
      j++
    ) {
      ...
      if (i != numofsnaketocompare) {
        for (let j = 0; j < this.snakesonfield[i].snakeBody.length; j++) {
          ...
          if (
            x === x1 &&
            y === y1 &&
            this.snakesonfield[numofsnaketocompare].headColor !=
              this.snakesonfield[i].headColor
          )
            return true;
        }
      }
  ...
  this.snakepregnantwillborn = function (pregnantsnake, numberofpregnantsnake) {
    let snaketemptobeborn = new SnakeObject(
      this.startSnakeLength,
      pregnantsnake.snakeBody[pregnantsnake.snakeBody.length - 1].getX(),
      pregnantsnake.snakeBody[pregnantsnake.snakeBody.length - 1].getY(),
      (pregnantsnake.snakeDirection + 2 + 4) % 4,
      pregnantsnake.headColor,
      pregnantsnake.cellBodysize,
      pregnantsnake.snakeEyeValues,
      pregnantsnake.mutations
    );
    snaketemptobeborn.snakeBody = pregnantsnake.snakeBody.slice(
      pregnantsnake.snakeBody.length - this.startSnakeLength - 1
    );
    snaketemptobeborn.snakeBody.reverse();
    //performing mutation------------------------------------------
    if (that.mutationsOn) {
      let snakeEyeValuesLength =
        ((2 * that.startwidthofeye + that.eyeDeltaWidth * (that.eyeDepth - 1)) /
          2) *
        that.eyeDepth;
      let countOfMutations = Math.floor(Math.random() * snakeEyeValuesLength);
      for (let i = 0; i < countOfMutations; i++) {
        let pointofmutation = Math.floor(
          Math.random() * (snakeEyeValuesLength + 1)
        );
        snaketemptobeborn.snakeEyeValues[pointofmutation] =
          Math.round(
            (snaketemptobeborn.snakeEyeValues[pointofmutation] +
              snaketemptobeborn.snakeEyeValues[pointofmutation] *
    ...
    pregnantsnake.snakeBody.splice(
      this.startSnakeLength,
      pregnantsnake.snakeBody.length - this.startSnakeLength - 1
    );
  };
  ...
  this.move = function () {
    if (!this.gameover) {
      for (let ii = 0; ii < this.snakesonfield.length; ii++) {
        ...
        let dir = this.snakesonfield[ii].calculateDirection(
          this.snakesonfield[ii],
          ii
        );
        if (dir === that.Updir) {
          y1 = (y1 - this.step + that.gameHeight) % that.gameHeight;
        }
        if (dir === that.Rightdir) {
          x1 = (x1 + this.step + that.gameWidth) % that.gameWidth;
        }
        if (dir === that.Downdir) {
          y1 = (y1 + this.step + that.gameHeight) % that.gameHeight;
        }
        ...
        //move snake
        this.snakesonfield[ii].snakeBody.unshift(
          new PointOfField(x1, y1, this.snakesonfield[ii].headColor)
        );
        if (this.isFood(x1, y1)) {
          this.eatFood(x1, y1);
          if (this.snakesonfield[ii].snakeBody.length >= this.maxSnakeLength) {
            this.snakesonfield[ii].snakeBody = this.snakesonfield[
              ii
            ].snakeBody.slice(0, -1);
